...
$log = Join-Path $env:NEW_BASELINE_RUNTIME 'train_queue.log'
$stamp = (Get-Date).ToString('o')
Add-Content -LiteralPath $log -Value "QUEUE_START $stamp"
& 'E:\Anaconda\envs\persist_stable_251\python.exe' -u (Join-Path $PSScriptRoot 'train_queue.py') *>> $log
$code = $LASTEXITCODE
Add-Content -LiteralPath $log -Value "QUEUE_EXIT $code $((Get-Date).ToString('o'))"
Set-Content -LiteralPath (Join-Path $env:NEW_BASELINE_RUNTIME 'train_queue_exit.txt') -Value "$code"
exit $code
