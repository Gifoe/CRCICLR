...
def main():
 ...
 ds=pd.DataFrame(drows);old_ds=pd.read_csv(OLD/'PHASE_A_DATASET_RESULTS.csv');oldrows=[]
 ...
 ds=ds.merge(pd.DataFrame(oldrows),left_on=['dataset','method'],right_on=['dataset','old_method'],how='left');ds.to_csv(OUT/'PHASE_A_DATASET_RESULTS.csv',index=False);js(OUT/'HARM_PROFILE.json',harm)
 diags=[]
 for d in ('OpenBMI','WBCIC'):
  for f in range(5):
 ...
 o,w=r('OpenBMI','BNLOCK-JOINTCE'),r('WBCIC','BNLOCK-JOINTCE');frozen_harm=float((s[(s.dataset=='WBCIC')&(s.method=='BNLOCK-JOINTCE')].FROZEN_LOGIT50_BA-s[(s.dataset=='WBCIC')&(s.method=='BNLOCK-JOINTCE')].EEGNet_BA<=-.01).mean());added=(w.BNLOCK_BA>=w.Frozen_LOGIT50_BA+.003) or ((frozen_harm-w.harm_le_minus_1pp)>=.05 and w.BNLOCK_BA>=w.Frozen_LOGIT50_BA-.002);openok=o.gain_vs_EEGNet_pp>=3 and o.positive_folds==5 and o.gain_vs_LOGIT50_pp>=-.5 and o.harm_le_minus_1pp<=.10;wok=w.gain_vs_EEGNet_pp>=1 and w.positive_folds>=4 and w.median_subject_delta_pp>0 and w.harm_le_minus_1pp<.15 and w.worst_fold_delta_pp>-2;primary=bool(openok and wok and added)
 ...
 elif wok and openok and not added:terminal='BNLOCK_REPAIRS_COLLAPSE_BUT_NO_ADDED_VALUE_STOP';preferred=None
 ...
 gate={'openbmi_primary':bool(openok),'wbcic_primary':bool(wok),'added_value':bool(added),'primary_pass':primary,'nrrf_adds_value':nrrf_add,'frozen_wbcic_harm_le_minus_1pp':frozen_harm,'terminal':terminal,'preferred_method':preferred};js(OUT/'PHASE_A_GATE.json',gate)
 ...
 lines+=['',f'BNLOCK-NRRF BA: OpenBMI {no.BNLOCK_BA:.4f}; WBCIC {n.BNLOCK_BA:.4f}.',f'BNLOCK-NRRF delta vs EEGNet: OpenBMI {no.gain_vs_EEGNet_pp:+.3f} pp; WBCIC {n.gain_vs_EEGNet_pp:+.3f} pp.',f'BN-lock fixed previous collapse: {"YES" if wok else "NO"}.',f'JOINT adds value beyond frozen fusion: {"YES" if added else "NO"}.',f'NRRF adds value beyond JOINT: {"YES" if nrrf_add else "NO"}.',f'Exact gate: {json.dumps(clean(gate),sort_keys=True)}.',f'Final terminal: **{terminal}**.']
