...
def two_tr_epoch(ctx: Mapping[str, Any], model: torch.nn.Module, opt: torch.optim.Optimizer, subjects: list[str], mean: np.ndarray, std: np.ndarray, dataset: str, seed: int, epoch: int, phase: str, objective: str) -> tuple[float, dict[str, int], list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    for step in range(nsteps):
        ...
        losses.append(float(total.detach().cpu())); metrics.append({"r1": float(r1.detach().cpu()), "r2": float(r2.detach().cpu()), "abs_r_diff": float(torch.abs(r1-r2).detach().cpu()), "dtraj": float(dtraj.detach().cpu()), "source_loss": float(lsrc.detach().cpu()), "pseudo_subjects": ";".join(meta_te), "partition_seed": pseed, "step": step, "pair_hash": ph})
...
def collect_outputs(contexts: dict[str, dict[str, Any]], selected: list[dict[str, Any]], refs: list[dict[str, Any]]) -> str:
    ...
    # Session exposure is rebuilt from the actual cache row construction.
    exposure=[]
    for dataset,ctx in contexts.items():
        fit=tuple(int(x) for x in geo.SESSIONS_FIT[dataset]); disc=(int(geo.SESSION_DISCOVERY[dataset]),)
        for method in METHODS:
            exposure.extend([
                {"dataset":dataset,"method":method,"stage":"selection_training","subject_role":"source_selection","sessions_used":";".join(map(str,fit)),"labels_used":True,"row_count":int(len(ctx["sel_rows"]))},
                {"dataset":dataset,"method":method,"stage":"source_validation","subject_role":"source_validation","sessions_used":";".join(map(str,disc)),"labels_used":True,"row_count":int(len(ctx["val_rows"]))},
                {"dataset":dataset,"method":method,"stage":"refit_training","subject_role":"source_refit","sessions_used":";".join(map(str,fit)),"labels_used":True,"row_count":int(len(ctx["refit_rows"]))},
                {"dataset":dataset,"method":method,"stage":"held_evaluation","subject_role":"held_subject","sessions_used":";".join(map(str,disc)),"labels_used":False,"row_count":int(len(ctx["held_rows"]))},
            ])
            if method != "B0_SUBJECT_BALANCED_ERM":
                exposure.append({"dataset":dataset,"method":method,"stage":"episodic_pseudo_unseen_training","subject_role":"source_pseudo_unseen","sessions_used":";".join(map(str,fit)),"labels_used":True,"row_count":int(len(ctx["refit_rows"]))})
    write_csv(ROOT/"SESSION_EXPOSURE_AUDIT.csv",exposure)
    expdf=pd.DataFrame(exposure); train=expdf[(expdf.stage.isin(["selection_training","refit_training","episodic_pseudo_unseen_training"])) & expdf.labels_used]
    wbcic2={m:bool(train[(train.dataset=="WBCIC")&(train.method==m)].sessions_used.str.split(";").apply(lambda x:"2" in x).any()) for m in METHODS}
    same=True
    for ds in DATASETS:
        groups=[set(train[(train.dataset==ds)&(train.method==m)].sessions_used) for m in METHODS]; same = same and len(groups)>0 and all(g==groups[0] for g in groups)
    fairness={"schema":SCHEMA,"WBCIC_source_session2_labels_used_by_ERM":wbcic2["B0_SUBJECT_BALANCED_ERM"],"WBCIC_source_session2_labels_used_by_2TR":wbcic2["B2_TWO_TRAJECTORY_MEAN"],"WBCIC_source_session2_labels_used_by_TSEG":wbcic2["B3_TSEG"],"same_labeled_session_universe":bool(same),"session_fairness_verified":bool(not any(wbcic2.values()) and same),"pass":bool(not any(wbcic2.values()) and same)}
    write_json(ROOT/"SESSION_FAIRNESS_GATE.json",fairness)
    # Matched-key mechanism analysis uses only the intersection of B2/TSEG keys.
    mechdf=pd.DataFrame(mech); mk=["dataset","opt_seed","phase","epoch","step","pair_hash"]; matched=[]
    if len(mechdf):
        a=mechdf[mechdf.method=="B2_TWO_TRAJECTORY_MEAN"]; b=mechdf[mechdf.method=="B3_TSEG"]; mm=a.merge(b,on=mk,suffixes=("_2TR","_TSEG"))
        for _,z in mm.iterrows(): matched.append({"dataset":z.dataset,"opt_seed":int(z.opt_seed),"phase":z.phase,"epoch":int(z.epoch),"step":int(z.step),"pair_hash":z.pair_hash,"delta_JS_TSEG_minus_2TR":float(z.dtraj_TSEG-z.dtraj_2TR),"delta_risk_dispersion_TSEG_minus_2TR":float(z.abs_r_diff_TSEG-z.abs_r_diff_2TR)})
    write_csv(ROOT/"MATCHED_MECHANISM_DELTAS.csv",matched); mdf=pd.DataFrame(matched)
    js_mean=float(mdf.delta_JS_TSEG_minus_2TR.mean()) if len(mdf) else None; js_med=float(mdf.delta_JS_TSEG_minus_2TR.median()) if len(mdf) else None; js_prop=float((mdf.delta_JS_TSEG_minus_2TR<0).mean()) if len(mdf) else None
    rd_mean=float(mdf.delta_risk_dispersion_TSEG_minus_2TR.mean()) if len(mdf) else None; rd_med=float(mdf.delta_risk_dispersion_TSEG_minus_2TR.median()) if len(mdf) else None; rd_prop=float((mdf.delta_risk_dispersion_TSEG_minus_2TR<0).mean()) if len(mdf) else None
    mechanism_ok=bool(len(mdf) and (js_prop>=.5 or rd_prop>=.5))
    two_d=[delta(d,"B2_TWO_TRAJECTORY_MEAN") for d in DATASETS]; tseg_d=[delta(d,"B3_TSEG") for d in DATASETS]
    gate_a=bool(float(delta("OpenBMI","B2_TWO_TRAJECTORY_MEAN").mean_delta_BA_pp)>=.5 and int(delta("OpenBMI","B2_TWO_TRAJECTORY_MEAN").positive_seeds)>=2)
    gate_b=bool(float(delta("WBCIC","B2_TWO_TRAJECTORY_MEAN").mean_delta_BA_pp)>=.5 and float(delta("WBCIC","B2_TWO_TRAJECTORY_MEAN").median_delta_BA_pp)>=.25 and int(delta("WBCIC","B2_TWO_TRAJECTORY_MEAN").positive_seeds)>=2)
    gate_c=bool(all(int(x.catastrophic_cells)==0 for x in two_d)); nonneg=sum(int(x.positive_seeds) for x in two_d); gate_d=bool(nonneg>=5 or (nonneg==4 and all(float(x.mean_delta_BA_pp)>=1 for x in two_d)))
    two_go=bool(fairness["pass"] and gate_a and gate_b and gate_c and gate_d)
    if two_go: terminal="STRONG_SESSION_FAIR_TWO_TRAJECTORY_SIGNAL" if all(float(x.mean_delta_BA_pp)>=1 for x in two_d) and nonneg>=5 else "SESSION_FAIR_TWO_TRAJECTORY_SIGNAL_SUPPORTED"
    elif float(delta("WBCIC","B2_TWO_TRAJECTORY_MEAN").mean_delta_BA_pp)<=0: terminal="TWO_TRAJECTORY_GAIN_EXPLAINED_OR_NOT_GENERAL"
    else: terminal="TWO_TRAJECTORY_SIGNAL_DATASET_SPECIFIC_ONLY"
    tseg_vs_2tr={d:float(delta(d,"B3_TSEG").mean_delta_BA_pp-delta(d,"B2_TWO_TRAJECTORY_MEAN").mean_delta_BA_pp) for d in DATASETS}; tseg_supported=bool(all(v>0 for v in tseg_vs_2tr.values()))
    gate={"schema":SCHEMA,"terminal":terminal,"gate_A_openbmi_replication":gate_a,"gate_B_wbcic_session_fair":gate_b,"gate_C_no_catastrophic_collapse":gate_c,"gate_D_pooled_two_trajectory":gate_d,"session_fairness_verified":bool(fairness["pass"]),"two_trajectory_go":two_go,"TSEG_specific_objective_supported":tseg_supported,"TSEG_SPECIFIC_REGULARIZATION_NOT_SUPPORTED":not tseg_supported,"matched_mechanism":{"rows":len(matched),"mean_delta_JS":js_mean,"median_delta_JS":js_med,"proportion_delta_JS_lt_0":js_prop,"mean_delta_risk_dispersion":rd_mean,"median_delta_risk_dispersion":rd_med,"proportion_delta_risk_dispersion_lt_0":rd_prop,"core_metric_reduction_supported":mechanism_ok},"canonical_outcome_labels_read":False,"OpenBMI_sealed_holdout_opened":False,"WBCIC_outer_10_opened":False}
    write_json(ROOT/"GO_GATE.json",gate)
    lines=["# Session-fair TSEG closure", "", "## Performance", "", "| Dataset | Method | Mean BA | Median BA | SD | Range |", "|---|---|---:|---:|---:|---:|"]
    for r in perf: lines.append(f"| {r['dataset']} | {r['method']} | {r['mean_BA']:.4f} | {r['median_BA']:.4f} | {r['seed_SD']:.4f} | {r['seed_range']:.4f} |")
    lines += ["", "## Relative", "", "| Dataset | 2TR-ERM mean | median | positive/3 | min 螖 | TSEG-ERM | TSEG-2TR |", "|---|---:|---:|---:|---:|---:|---:|"]
    for d in DATASETS: lines.append(f"| {d} | {delta(d,'B2_TWO_TRAJECTORY_MEAN').mean_delta_BA_pp:+.4f} | {delta(d,'B2_TWO_TRAJECTORY_MEAN').median_delta_BA_pp:+.4f} | {int(delta(d,'B2_TWO_TRAJECTORY_MEAN').positive_seeds)}/3 | {delta(d,'B2_TWO_TRAJECTORY_MEAN').minimum_delta_BA_pp:+.4f} | {delta(d,'B3_TSEG').mean_delta_BA_pp-delta(d,'B0_SUBJECT_BALANCED_ERM').mean_delta_BA_pp if False else delta(d,'B3_TSEG').mean_delta_BA_pp:+.4f} | {tseg_vs_2tr[d]:+.4f} |")
    lines += ["", f"Terminal: `{terminal}`", "", f"Gates: A={gate_a}, B={gate_b}, C={gate_c}, D={gate_d}, session_fairness={fairness['pass']}.", "", f"WBCIC source session2 labels used during training: {any(wbcic2.values())}.", f"Matched mechanism rows: {len(matched)}; mean 螖JS={js_mean}; mean 螖risk-dispersion={rd_mean}.", f"TSEG-specific regularization supported: {tseg_supported}.", "Held-out evaluation was post-training only; no sealed holdout or WBCIC outer-10 was opened.", "Only OpenBMI/WBCIC fold0, EEGNet, seeds 0/1/2 were run."]
