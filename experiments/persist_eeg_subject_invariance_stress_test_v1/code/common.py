"""Shared implementation for the frozen OpenBMI subject-invariance stress test.
...
EXP = HERE.parent
REPO = HERE.parents[2]
PROTOCOL_PATH = EXP / "STRESS_TEST_PROTOCOL_FROZEN.json"
RESULTS = EXP / "results"
FIGURES = EXP / "figures"
RUNTIME = EXP / "runtime"
RUNS = RUNTIME / "runs"
...
def protocol() -> dict[str, Any]:
    ...
    if value.get("schema") != "PERSIST_EEG_SUBJECT_INVARIANCE_STRESS_TEST_V1":
        raise RuntimeError("unexpected stress-test protocol schema")
    ...
    if value.get("repository_start_sha") != "3366d419cfd5d26e88f5de3d751199d9068b649e":
...
def frozen_fold(fold: int) -> dict[str, tuple[str, ...]]:
    ...
    validation = tuple(subject_sort(row["inner_validation"]))
    train = tuple(subject_sort(pool - set(outcome) - set(validation)))
    source = tuple(subject_sort(pool - set(outcome)))
    if (len(outcome), len(validation), len(train), len(source)) != (8, 8, 24, 32):
        raise RuntimeError(f"fold {fold} cardinality failure")
    if set(outcome) & set(source) or set(train) & set(validation):
        raise RuntimeError(f"fold {fold} subject overlap")
    return {"outcome": outcome, "inner_validation": validation, "inner_train": train, "source": source}


def _cache_candidates() -> list[Path]:
    values: list[Path] = []
    explicit = os.environ.get("PERSIST_STRESS_DATA_CACHE", "").strip()
    if explicit:
...
def locate_authorized_cache() -> Path:
    for root in _cache_candidates():
        ...
        if signal.is_file() and metadata.is_file():
            return root
    raise FileNotFoundError("authorized 40-subject OpenBMI cache not found; set PERSIST_STRESS_DATA_CACHE")
...
def load_data() -> DevelopmentData:
    ...
    valid = bool(
        ...
        and set(map(int, cells.tolist())) == {50}
