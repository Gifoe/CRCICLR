#!/usr/bin/env python3
"""Seed-0, five-fold LiteBN v2 ablation, excluding matched-Full training."""
...
def build_variant(task: str, variant: str, device):
    ...
    del reference
    set_seed()
    return LiteBNAblationV2(channels, classes, variant).to(device)
...
def evaluate_all(device):
    ...
    for task in TASKS:
        ...
        for fold in fold_map[base.TASKS[task]["dataset"]]:
            ...
            atomic_csv(OUT / "SUBJECT_RESULTS.csv", subject_rows)
...
def summarize(subject_rows):
    ...
    for variant in ALL_VARIANTS:
        ...
        for task in TASKS:
            ...
            full = biological[(biological.task == task) & (biological.variant == REFERENCE)].set_index("subject_id")
        ...
        rows.append(row)
    atomic_csv(OUT / "ABLATION_SUMMARY.csv", rows)
    return rows
...
def report(summary):
    ...
    lines = [
        "# LiteBN ablation v2 seed-0 report", "",
        "User override: B0_FULL_MATCHED was not trained. OFFICIAL_FINAL_LITEBN_REFERENCE is a frozen non-matched reference and primary matched-B0 causal claims are unavailable.", "",
    ...
    (OUT / "FINAL_LITEBN_ABLATION_V2_SEED0_REPORT.md").write_text("\n".join(lines), encoding="utf-8")
...
def add_full_rows(records):
    ...
    for task in TASKS:
        for fold in range(5):
            if (task, REFERENCE, fold) in existing:
                continue
            checkpoint = full_checkpoint(task, fold)
            records.append({
                "model": "LiteBN", "task": task, "variant": REFERENCE, "seed": SEED,
                ...
                "history": [], "reused_protocol_identical": False,
                "reference_only": True,
            })
...
def main():
    ...
    for directory in (OUT, RUN, PROTOCOL):
        directory.mkdir(parents=True, exist_ok=True)
    atomic_json(PROTOCOL / "PROTOCOL.json", {
        ...
        "user_override": "Do not train B0_FULL_MATCHED; train B1-B4 only",
        "matched_full_available": False,
        "reference": REFERENCE,
    ...
    for task in TASKS:
        for fold in fold_map[base.TASKS[task]["dataset"]]:
            ...
            for variant in pending:
                ...
                atomic_json(log_path, records)
                atomic_csv(OUT / "ABLATION_RUNS.csv", [{key: value for key, value in row.items() if key != "history"} for row in records])
    ...
    report(summary)
    atomic_json(OUT / "FINAL_LITEBN_ABLATION_V2_STATUS.json", {
    ...
    print("LITEABL_V2_COMPLETE", flush=True)
