...
class GumbelGroupEmbedding(nn.Module):
    def __init__(self, C, d_model, max_groups=8, temperature_init=1.0, temperature_min=0.1, anneal_rate=0.0003, sim_loss_weight=0.1):
        ...
        # The release hard-codes a TDBRAIN-only 33-variable file. For a new
        # variable count, use the same one-hot-plus-N(0,0.1) initialization
        # family as the official grouping utility, without EEG coordinates or
        # any validation/outer/heldout data dependency.
        groups_matrix = np.zeros((C, self.G), dtype=np.float32)
        groups_matrix[np.arange(C), np.arange(C) % self.G] = 1.0
        groups_matrix = np.clip(groups_matrix + np.random.normal(0.0, 0.1, groups_matrix.shape), 0.0, 1.0)
        groups_matrix = torch.tensor(groups_matrix, dtype=torch.float32)
