"""Frozen seed-0 PEEH audit for the EEGConformer and FBCNet baselines.
...
BASELINE_CODE = Path(os.environ.get(
    "EEGCONFORMER_FBCNET_CODE",
    str(EXP.parent / "persist_eeg_eegconformer_fbcnet_multiseed_v1" / "code"),
))
BASELINE_RUNTIME = Path(os.environ.get(
    "EEGCONFORMER_FBCNET_RUNTIME", r"D:\nips-temp\TotalP\P1\eegconformer_fbcnet_runtime"
))
...
def cell(model: str, task: str, fold: int) -> tuple[dict[str, Any], Path]:
    root = BASELINE_RUNTIME / "cells" / model.lower() / task.lower() / f"fold{fold}_seed0"
...
def all_checkpoint_rows() -> list[dict[str, Any]]:
    ...
    if len(rows) != 40: raise RuntimeError(f"expected 40 frozen cells, found {len(rows)}")
    return rows


def prelock() -> None:
    rows = all_checkpoint_rows()
    lock = {"schema": "PERSIST_EEG_EEGCONFORMER_FBCNET_PEEH_LOCK_V1", "created_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
            "models": list(MODELS), "scope": "frozen checkpoint replay only; no model training or checkpoint selection",
    ...
    print("PEEH_PROTOCOL_LOCKED_40_FROZEN_RUNS", flush=True)
...
def build_model(row: dict[str, Any], device: torch.device) -> tuple[torch.nn.Module, torch.nn.Module]:
    ...
    models = module("eegconformer_fbcnet_peeh_models", BASELINE_CODE / "models.py")
    model = models.build_model(model_name, channels=row["channels"], samples=row["samples"], classes=row["classes"])
    ...
    if model_name == "EEGConformer": head = model.classifier[-1]
    elif model_name == "FBCNet": head = model.head
...
def representations(model: torch.nn.Module, head: torch.nn.Module, x: np.ndarray, model_name: str, device: torch.device, batch: int = 64) -> np.ndarray:
    """Capture the frozen pre-classifier representation in bounded batches.

    FBCNet's nine-band adapter is deliberately applied *inside* the loop: a
    whole-fold [N,9,C,T] allocation would be a multi-GB CPU transient.  Each
    batch calls the same deterministic ``filterbank.transform`` used during
    the original FBCNet training/evaluation, so this changes memory use only.
    """
    ...
    try:
        with torch.inference_mode():
            for start in range(0, len(x), batch):
                value = resample(model_name, x[start:start+batch])
                if model_name == "FBCNet":
                    filterbank = module("eegconformer_fbcnet_peeh_filterbank", BASELINE_CODE / "filterbank.py")
                    value = filterbank.transform(value)
                captured.clear(); model(torch.from_numpy(value).to(device))
...
def aggregate() -> None:
    ...
    expected_invalid: list[dict[str, Any]] = []
    ...
    for model in MODELS:
        for task in TASKS:
            r = next((x for x in run_rows if x["Model"] == model and x["Task"] == task), None)
            if r is not None:
                reps.append({"Model":model,"Task":task,"Representation dim":r["Representation dim"],"trainable_parameters":r["trainable_parameters"]})
    write_csv(OUT/"REPRESENTATION_SPEC.csv",reps)
    write_json(OUT/"MISSING_INVALID_CELLS.json",{"missing":missing,"expected_protocol_invalid":expected_invalid,"unexpected_missing":unexpected_missing,"reason":"no protocol-invalid exceptions are pre-authorized for this two-model extension"})
    ...
    valid=(len(cells)==40 and not missing and not unexpected_missing and sum(x["Status"]=="COMPLETE" for x in primary)==8 and finite)
    write_json(OUT/"VALIDATION.json",{"pass":valid,"expected_cells":40,"analyzed_cells":len(cells),"complete_model_tasks":sum(x["Status"]=="COMPLETE" for x in primary),"protocol_invalid_cells":[],"unexpected_missing":unexpected_missing,"finite_metrics":finite,"bootstrap_draws":BOOTSTRAP_DRAWS,"training_performed":False,"batchnorm_updated":False,"probe_standardizer":"fit once on intact TRAIN and fixed across interventions"})
    lines=["# Frozen EEGConformer/FBCNet PEEH (seed 0)","","This extension replays 40 previously selected frozen checkpoints. It does not train models, change a checkpoint, update batch-normalization, or reselect a model.","","|Model|Task|Params|D|Intact BA|Protected BA|Random BA|PEEH pp|95% CI|Coverage|Significant|","|---|---|---:|---:|---:|---:|---:|---:|---|---:|---|"]
    for x in primary:
        p=next((r["trainable_parameters"] for r in reps if r["Model"]==x["Model"] and r["Task"]==x["Task"]), 0)
    ...
    print("EEGCONFORMER_FBCNET_PEEH_40_FROZEN_RUNS_COMPLETE",flush=True)
