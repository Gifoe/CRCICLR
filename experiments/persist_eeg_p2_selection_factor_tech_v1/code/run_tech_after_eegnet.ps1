...
$code = Join-Path $repo 'experiments/persist_eeg_p2_selection_factor_tech_v1/code'
$runtime = 'D:/nips-temp/TotalP/P1/p2_selection_factor_tech_runtime'
$eegRuntime = 'D:/nips-temp/TotalP/P1/p2_selection_factor_eegnet_runtime'
$eegValidation = Join-Path $repo 'experiments/persist_eeg_p2_selection_factor_eegnet_v1/protocol/VALIDATION.json'
...
$log = Join-Path $logDir 'queue.out.log'
$err = Join-Path $logDir 'queue.err.log'

while ($true) {
    if ((Test-Path (Join-Path $eegRuntime 'COMPLETE.txt')) -and (Test-Path $eegValidation)) {
        try {
            $validation = Get-Content -LiteralPath $eegValidation -Raw | ConvertFrom-Json
            if ($validation.pass -eq $true) { break }
        } catch {}
    }
    Start-Sleep -Seconds 30
}
Set-Content -LiteralPath (Join-Path $runtime 'EEGNET_PREDECESSOR_VALIDATED.txt') -Value (Get-Date -Format o)
Set-Location $code

& $python -u run_p2_selection_factor.py --stage protocol 1>> $log 2>> $err
if ($LASTEXITCODE -ne 0) { throw 'TeCh P2 protocol match validation failed' }

$dry = Join-Path $runtime 'cells/openbmi_mi/fold0_seed0.json'
if (!(Test-Path -LiteralPath $dry)) {
    & $python -u run_p2_selection_factor.py --stage dry-run --task OpenBMI_MI --fold 0 1>> $log 2>> $err
    if ($LASTEXITCODE -ne 0 -or !(Test-Path -LiteralPath $dry)) { throw 'TeCh P2 one-cell dry-run failed' }
}
...
foreach ($task in @('OpenBMI_MI', 'OpenBMI_ERP', 'OpenBMI_SSVEP', 'WBCIC_MI')) {
    foreach ($fold in 0..4) {
        ...
        foreach ($attempt in 1..3) {
            & $python -u run_p2_selection_factor.py --stage cell --task $task --fold $fold 1>> $log 2>> $err
            $exit = $LASTEXITCODE
            if ($exit -eq 0 -and (Test-Path -LiteralPath $cell)) { $ok = $true; break }
            Add-Content -LiteralPath $err -Value ("RETRY {0}/fold{1} attempt={2} exit={3}" -f $task, $fold, $attempt, $exit)
            Start-Sleep -Seconds 3
        }
        if (!$ok) { throw "TeCh P2 cell failed: $task/fold$fold" }
    }
}

& $python -u run_p2_selection_factor.py --stage aggregate 1>> $log 2>> $err
if ($LASTEXITCODE -ne 0) { throw 'TeCh P2 aggregation failed' }
