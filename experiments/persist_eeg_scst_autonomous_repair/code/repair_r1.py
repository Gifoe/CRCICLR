...
def _gate(frame: pd.DataFrame, geometry: pd.DataFrame, match: pd.DataFrame) -> dict[str, object]:
    ...
    source_pass = bool(any(row["pass"] for row in recipe_rows) and all(any(row["checks"].get(f"{dataset}_delta_ge_002", False) for row in recipe_rows) for dataset in c.DATASETS) and subject_fraction >= 0.60 and all(value["checks"]["target_affinity_ci_lower_positive"] and value["checks"]["coverage"] and value["checks"]["class_fidelity"] and value["checks"]["displacement"] for value in affinity.values()))
