...
try:
    ...
    # MedformerLayer does not use Reformer. Keep the unrelated optional class
    # importable without adding an unused dependency to this frozen baseline.
    class LSHSelfAttention:  # pragma: no cover - forbidden unused path
        def __init__(self, *args, **kwargs):
            raise ImportError("reformer_pytorch is required only for the unused ReformerLayer")
try:
    from einops import rearrange, repeat
except ImportError:
    # These helpers are referenced only by an unrelated Crossformer layer.
    def rearrange(*args, **kwargs):
        raise ImportError("einops is required only for the unused Crossformer layer")
