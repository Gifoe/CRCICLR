...
def main():
    ...
    if (c.RESULTS/'FM_SCST_SUMMARY.csv').is_file():
        scst=pd.read_csv(c.RESULTS/'FM_SCST_SUMMARY.csv')
        expected={'gate_FM_task_competence','gate_independent_probe_competence','gate_stability','gate_subject_fidelity','gate_class_fidelity','gate_manifold','valid'}
        if not expected.issubset(scst.columns):errors.append('SCST frozen gates missing')
