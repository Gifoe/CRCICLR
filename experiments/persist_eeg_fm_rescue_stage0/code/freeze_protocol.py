...
def main() -> None:
    ...
    lock={"schema":"FM_RESCUE_STAGE0_PROTOCOL_LOCK_V1","created_at_code_commit":c.git_head(),"frozen_before_primary_outcomes":True,
        ...
        "SCAA":{"dataset":"WBCIC","roles":{"S1":0,"S2":1,"S3":2},"adapter":"classification head only; encoder and normalization frozen","S1_split":"within-class chronological 70/30","selected_lr":{r.fm:float(r.lr) for r in scaa.itertuples()},"certificate":"Delta2>0","threshold_search":False,"not_materially_worse_than_anchor_tolerance_BA":.01,"constructive_requires_task_competent_FMs":True,"pooled_sign_evidence":"subject-grouped across FMs; 10,000 bootstrap CI lower > 0.5","bootstrap":10000},
