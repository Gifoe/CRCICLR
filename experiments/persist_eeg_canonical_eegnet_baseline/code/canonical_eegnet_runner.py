...
def aggregate(payloads: list[dict[str, Any]], roles_by_dataset: dict[str, list[dict[str, list[str]]]], data_by_dataset: dict[str, DatasetData]) -> None:
    ...
    for dataset in DATASETS:
        ...
        summary_rows.append({"dataset": dataset, "summary_type": "primary_three_seed_probability_mean", "mean_subject_BA": float(primary.BA.mean()), "median_subject_BA": float(primary.BA.median()), "subject_SD": float(primary.BA.std(ddof=1)), "bootstrap_CI95_L": ci_l, "bootstrap_CI95_U": ci_u, "mean_accuracy": float(primary.accuracy.mean()), "mean_macro_F1": float(primary.macro_F1.mean()), "mean_NLL": float(primary.NLL.mean()), "n_subjects": int(len(primary)), "folds": 5, "seeds": 3, "mean_best_epoch": float(s.best_epoch.mean()), "historical_reference_BA": HISTORICAL_REFERENCE[dataset], "difference_vs_historical_pp": float(100 * (primary.BA.mean() - HISTORICAL_REFERENCE[dataset]))})
        summary_rows.append({"dataset": dataset, "summary_type": "secondary_mean_single_seed_aggregate", "mean_subject_BA": float(seed_means.mean()), "median_subject_BA": float(s.BA.median()), "subject_SD": float(seed_means.std(ddof=1)), "bootstrap_CI95_L": None, "bootstrap_CI95_U": None, "mean_accuracy": float(s.accuracy.mean()), "mean_macro_F1": float(s.macro_F1.mean()), "mean_NLL": float(s.NLL.mean()), "n_subjects": int(len(s)), "folds": 5, "seeds": 3, "mean_best_epoch": float(s.best_epoch.mean()), "historical_reference_BA": HISTORICAL_REFERENCE[dataset], "difference_vs_historical_pp": float(100 * (seed_means.mean() - HISTORICAL_REFERENCE[dataset]))})
...
def write_final_reports(payloads: list[dict[str, Any]], device: torch.device) -> None:
    ...
    report = {"status": "CANONICAL BASELINE STATUS", "protocol": "CANONICAL_VANILLA_EEGNET_BASELINE_V1", "terminal": "CANONICAL_EEGNET_BASELINE_ESTABLISHED", "validity_answers": validity_answers, "summary": stats, "payload_count": len(payloads), "outer_status": {"OpenBMI_internal_holdout_accessed": False, "WBCIC_outer_10_accessed": False}, "deliverables": ["results/BASELINE_SUMMARY.csv", "results/PER_SUBJECT_RESULTS.csv", "results/TRIAL_PREDICTIONS.csv", "results/PER_FOLD_RESULTS.csv", "results/SEED_SUMMARY.csv", "results/CANONICAL_BASELINE_STATISTICS.json", "FINAL_BASELINE_REPORT.md", "FINAL_BASELINE_REPORT.json", "CANONICAL_PROTOCOL.md", "BASELINE_LEGALITY_AUDIT.md", "BASELINE_PROVENANCE_AUDIT.md", "HISTORICAL_REFERENCE_AUDIT.md"]}
