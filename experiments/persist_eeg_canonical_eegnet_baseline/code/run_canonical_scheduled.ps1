...
$exp = Join-Path $repo 'experiments\persist_eeg_canonical_eegnet_baseline'
$runtime = Join-Path $exp 'runtime'
$stdout = Join-Path $runtime 'CANONICAL_RUN.log'
$stderr = Join-Path $runtime 'CANONICAL_RUN.err.log'
$exitPath = Join-Path $runtime 'CANONICAL_RUN.exit.json'
...
Set-Location $repo
$script = Join-Path $exp 'code\canonical_eegnet_runner.py'
