...
class GQA(nn.Module):
 ...
 def forward(self,x):
  b,l,_=x.shape; h=self.n1(x); q=rope(self.q(h).view(b,l,8,8).transpose(1,2)); k=rope(self.k(h).view(b,l,4,8).transpose(1,2)).repeat_interleave(2,1); v=self.v(h).view(b,l,4,8).transpose(1,2).repeat_interleave(2,1); a=(q@k.transpose(-1,-2)/math.sqrt(8)).softmax(-1); self.ent=(-(a.clamp_min(1e-12)*a.clamp_min(1e-12).log()).sum(-1)).mean().detach(); z=x+self.ga*self.dp(self.o((F.dropout(a,.1,self.training)@v).transpose(1,2).reshape(b,l,64))); return z+self.gf*self.dp(self.ff(self.n2(z)))
class ConvMixer(nn.Module):
 ...
 def forward(self,x):
  z=F.glu(self.fc(self.n(x)),-1).transpose(1,2); z=self.pw(F.gelu(self.dw(z))).transpose(1,2); self.active=z.detach().norm(); return x+self.g*self.dp(F.dropout(z,.1,self.training))
...
class Cross(nn.Module):
 ...
 def forward(self,t,s):
  b,l,_=t.shape;q=self.q(self.nt(t)).view(b,l,4,16).transpose(1,2);k=self.k(self.ns(s)).view(b,288,4,16).transpose(1,2);v=self.v(self.ns(s)).view(b,288,4,16).transpose(1,2);a=(q@k.transpose(-1,-2)/4).softmax(-1);self.ent=(-(a.clamp_min(1e-12)*a.clamp_min(1e-12).log()).sum(-1)).mean().detach();self.mass=a.detach().mean((0,1,2));return t+self.g*self.o((F.dropout(a,.1,self.training)@v).transpose(1,2).reshape(b,l,64))
