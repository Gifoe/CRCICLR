...
$runtime = 'D:\nips-temp\TotalP\P1\baseline_metrics_closure_v1_native_batch_runtime'
$log = Join-Path $runtime 'native_batch_inference.log'
$env:CLOSURE_RUNTIME = $runtime
$env:CLOSURE_NATIVE_BATCH = '1'
...
try {
    foreach ($model in @('ModernTCN', 'Medformer')) {
        ...
        if ($LASTEXITCODE -ne 0) { throw "Native-batch frozen inference failed: $model exit=$LASTEXITCODE" }
    }
