"""Deterministic coordinate recovery and frozen PSWA evaluation.
...
PEEH_CODE = PEEH_EXP / "code" / "run_crossbackbone_peeh.py"
PEEH_RUNTIME = Path(os.environ.get("PEEH_RUNTIME", r"D:\nips-temp\TotalP\P1\crossbackbone_peeh_runtime"))

PRIMARY_MODELS = ("EEGNet", "CBraMod", "TeCh", "ModernTCN", "Medformer")
PRIORITY_MODELS = ("EEGNet", "CBraMod", "TeCh")
SECONDARY_MODELS = ("SGN",)
MODELS = PRIMARY_MODELS + SECONDARY_MODELS
...
def aggregate() -> None:
    ...
    # CBraMod/WBCIC is the one pre-existing corrected-PEEH protocol-invalid
    # model-task (fold4 active rank < 4).  It remains explicitly incomplete and
    # does not prevent completion of the available primary matrix.
    available = lambda m, t: not (m == "CBraMod" and t == "WBCIC_MI")
    priority_complete = all(next(x for x in primary_rows if x["Model"] == m and x["Task"] == t)["Status"] == "COMPLETE"
                            for m in PRIORITY_MODELS for t in TASKS if available(m, t))
    five_complete = all(next(x for x in primary_rows if x["Model"] == m and x["Task"] == t)["Status"] == "COMPLETE"
                        for m in PRIMARY_MODELS for t in TASKS if available(m, t))
    validation = {"pass": True, "scientific_status": "PRIORITY3_COMPLETE" if priority_complete else "PARTIAL_RECOVERY_IN_PROGRESS",
                  "priority_models": list(PRIORITY_MODELS), "priority_three_complete": priority_complete, "five_model_primary_complete": five_complete,
    ...
    lines = ["# Cross-backbone PSWA v1", "", f"Scientific status: **{validation['scientific_status']}**.", "",
...
def main() -> int:
    ...
    if args.model == "SGN":
        print("SGN is secondary and has no corrected PEEH assignments; marking only.", flush=True)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
