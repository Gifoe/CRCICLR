...
The committed `codex/persist-eeg-crossbackbone-pswa-v1` CSV marks even EEGNet/TeCh PSWA rows `NOT_ESTIMATED`, because the required random/projection artifacts were not persisted in that commit. A separate local working tree contains recovered PSWA rows and random controls, but those files and recovery code are **uncommitted** and materially differ from the committed artifacts. Therefore the requested regression against *committed* PSWA values, and a provenance-clean ModernTCN/Medformer PSWA extension, cannot be certified from the committed branch. No PSWA value was imported into a final table.
