"""AMSE-v1 seed-0 frozen five-fold experiment.
...
def restore_rng(s: dict[str, Any]) -> None:
    ...
    if "cuda" in s and torch.cuda.is_available(): torch.cuda.set_rng_state_all(s["cuda"])
...
class AMSE(torch.nn.Module):
    """Anchored Multi-Scale Evidence Network v1, exactly as protocol specification."""
    def __init__(self, channels: int, classes: int = 2):
        ...
        self.short = stem(15); self.anchor = stem(64); self.long = stem(127)
        self.stable_depth = torch.nn.Conv2d(16, 16, (1, 16), padding="same", groups=16, bias=False)
        self.stable_point = torch.nn.Conv2d(16, 16, 1, bias=False)
        self.stable_bn = torch.nn.BatchNorm2d(16)
        self.stable_fc = torch.nn.Linear(128, 64)
        self.stable_ln = torch.nn.LayerNorm(64)
        self.stable_head = torch.nn.Linear(64, classes)
    ...
    def forward(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        x = x.unsqueeze(1)
        h15 = self._stem(x, self.short, self.drop_stem, self.training)
        h64 = self._stem(x, self.anchor, self.drop_stem, self.training)
        h127 = self._stem(x, self.long, self.drop_stem, self.training)
        y = self.stable_depth(h64)
        y = self.stable_point(y); y = torch.nn.functional.elu(self.stable_bn(y))
        y = torch.nn.functional.avg_pool2d(y, (1, 8)); y = torch.nn.functional.dropout(y, .25, self.training)
        y = torch.nn.functional.adaptive_avg_pool2d(y, (1, 8)).flatten(1)
        hs = torch.nn.functional.dropout(torch.nn.functional.elu(self.stable_ln(self.stable_fc(y))), .25, self.training)
        zs = self.stable_head(hs)
        he = torch.cat((h15, h64.detach(), h127), dim=1)
    ...
    for epoch in range(start, EPOCHS + 1):
        ...
        for ep in manifest[epoch - 1]:
            ...
            with torch.autocast(device_type=device.type, dtype=torch.float16, enabled=amp):
                zf, zs, ze = model(x); loss = torch.nn.functional.cross_entropy(zf, y) + .25 * torch.nn.functional.cross_entropy(zs, y) + .25 * torch.nn.functional.cross_entropy(ze, y)
        ...
        row = {"epoch": epoch, "CE_total": float(np.mean(losses)), "inner_val_subject_BA": val_ba, "selected": selected}; history.append(row)
...
def lock_only(device: torch.device) -> None:
    ...
    spec = {"name": "AMSE-v1", "input": "[B,C,T]", "stems": {"short": {"kernel": 15, "out": 16, "pool": 4, "dropout": .20}, "anchor": {"kernel": 64, "out": 16, "pool": 4, "dropout": .20}, "long": {"kernel": 127, "out": 16, "pool": 4, "dropout": .20}}, "stable": {"uses": ["H64"], "depthwise_kernel": 16, "channels": 16, "pool": 8, "adaptive": [1, 8], "embedding": 64, "dropout": [.25, .25]}, "expressive": {"uses": ["H15", "detach(H64)", "H127"], "channels": 48, "depthwise_kernels": [15, 31], "pointwise_channels": [64, 64], "pool": [2, 2], "adaptive": [1, 8], "embedding": 64, "dropout": [.15, .15, .25]}, "decision": "0.5*stable_logits + 0.5*expressive_logits", "learned_gate": False, "temperature": False, "classes": 2}
    write_json(PROTOCOL / "AMSE_MODEL_SPEC.json", spec); (PROTOCOL / "AMSE_MODEL_SPEC.md").write_text("# AMSE-v1 model lock\n\nExact architecture is recorded in `AMSE_MODEL_SPEC.json`. The anchor H64 path is the only stable input; expressive input is `concat(H15, stop_gradient(H64), H127)`. Final logits are a fixed 50/50 raw-logit average.\n", encoding="utf-8")
    training = {"seed": 0, "datasets": ["OpenBMI", "WBCIC"], "folds": 5, "epochs": 60, "minimum_selection_epoch": 10, "optimizer": "AdamW", "lr": LR, "weight_decay": WD, "gradient_clip": CLIP, "loss": "CE(final)+0.25*CE(stable)+0.25*CE(expressive)", "selection": "inner_val future-session subject-equal BA", "amp": device.type == "cuda", "batch": "frozen carrier episode manifest", "holdout_access": False}
    ...
    entries = []
    for p in (PROTOCOL / "AMSE_MODEL_SPEC.json", PROTOCOL / "TRAINING_PROTOCOL.json", PROTOCOL / "SPLIT_PROVENANCE.json", PROTOCOL / "REFERENCE_PROVENANCE.json", PROTOCOL / "OUTCOME_VISIBILITY_AUDIT.json", PROTOCOL / "SMOKE_TEST.json"):
...
def run_experiment(device: torch.device) -> None:
    ...
    for dataset in ("OpenBMI", "WBCIC"):
        ...
        for fold in folds[dataset]:
            ...
            set_seed(SEED + f); model = AMSE(bundle.channels).to(device); info = train(model, dataset, fold, manifest, minfo, bundle, cache, device); training_rows.append({"dataset": dataset, "fold": f, "normalizer": norm, "manifest": minfo, **info})
            ...
            for subject in [str(x) for x in fold["outer_dev_subjects"]]:
                ...
                row["AMSE_Final_minus_best_ref_pp"] = (row["AMSE_Final_BA"] - max(row["EEGNet_BA"], row["LiteBN_BA"])) * 100
    ...
    for dataset in ("OpenBMI", "WBCIC"):
        ds = sdf[sdf.dataset == dataset]; ff = fdf[fdf.dataset == dataset]; best_ref = float(ds[["EEGNet_BA", "LiteBN_BA"]].max(axis=1).mean()); ref50 = float(ds.REF50_BA.mean()); final = float(ds.AMSE_Final_BA.mean()); stable = float(ds.AMSE_Stable_BA.mean()); expressive = float(ds.AMSE_Expressive_BA.mean()); gref = (ref50 - best_ref) * 100; gamse = (final - best_ref) * 100
        boot_best = bootstrap_delta((ds.AMSE_Final_BA.to_numpy() - ds[["EEGNet_BA", "LiteBN_BA"]].max(axis=1).to_numpy()) * 100); boot_ref50 = bootstrap_delta((ds.AMSE_Final_BA.to_numpy() - ds.REF50_BA.to_numpy()) * 100)
        r = gamse / gref if gref > 0 else None; posfold = int((ff.AMSE_Final_minus_best_ref_pp > 0).sum())
        summaries.append({"dataset": dataset, "EEGNet_BA": float(ds.EEGNet_BA.mean()), "LiteBN_BA": float(ds.LiteBN_BA.mean()), "REF50_BA": ref50, "AMSE_Stable_BA": stable, "AMSE_Expressive_BA": expressive, "AMSE_Final_BA": final, "best_single_ref_BA": best_ref, "G_REF_pp": gref, "G_AMSE_pp": gamse, "R": r, "G_INTERNAL_pp": (final - max(stable, expressive)) * 100, "final_positive_folds": posfold, "final_bootstrap_ci_low_pp": boot_best["ci_low_pp"], "final_bootstrap_ci_high_pp": boot_best["ci_high_pp"], "final_vs_REF50_ci_low_pp": boot_ref50["ci_low_pp"], "final_vs_REF50_ci_high_pp": boot_ref50["ci_high_pp"]})
    ...
    sdf.to_csv(OUT / "SUBJECT_METRICS.csv", index=False); fdf.to_csv(OUT / "FOLD_METRICS.csv", index=False); summary_df.to_csv(OUT / "DATASET_SUMMARY.csv", index=False); pd.DataFrame(branch_rows).to_csv(OUT / "BRANCH_COMPETENCE.csv", index=False); pd.DataFrame(fusion_rows).to_csv(OUT / "FUSION_GAIN_RETENTION.csv", index=False); cdf.to_csv(OUT / "COMPLEMENTARITY.csv", index=False); write_json(OUT / "MODEL_COST.json", {"EEGNet_params_62": count(EEGNet(62)), "EEGNet_params_58": count(EEGNet(58)), "LiteBN_params_62": count(carrier.CompactLite(62, "bn")), "LiteBN_params_58": count(carrier.CompactLite(58, "bn")), "AMSE_params_62": count(AMSE(62)), "AMSE_params_58": count(AMSE(58)), "AMSE_vs_sum": {"62": count(AMSE(62)) < count(EEGNet(62)) + count(carrier.CompactLite(62, "bn")), "58": count(AMSE(58)) < count(EEGNet(58)) + count(carrier.CompactLite(58, "bn"))}}); write_json(OUT / "REFERENCE_AUDIT.json", {"entries": ref_audit, "holdout_loaded": False}); write_json(OUT / "CHECKPOINT_SELECTION.json", training_rows); write_json(OUT / "TRAINING_LOGS.json", training_rows)
    lines = ["# AMSE-v1 seed-0 decision", "", f"Final terminal: **{terminal}**", "", "| Dataset | EEGNet | LiteBN | REF50 | Stable | Expressive | AMSE Final | Δ vs best ref | Δ vs REF50 | R |", "|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|"]
    for r in summaries:
        rtxt = "NA" if r["R"] is None else f"{r['R']:.3f}"
        lines.append(f"| {r['dataset']} | {r['EEGNet_BA']*100:.2f}% | {r['LiteBN_BA']*100:.2f}% | {r['REF50_BA']*100:.2f}% | {r['AMSE_Stable_BA']*100:.2f}% | {r['AMSE_Expressive_BA']*100:.2f}% | {r['AMSE_Final_BA']*100:.2f}% | {r['G_AMSE_pp']:+.2f} pp | {(r['AMSE_Final_BA']-r['REF50_BA'])*100:+.2f} pp | {rtxt} |")
