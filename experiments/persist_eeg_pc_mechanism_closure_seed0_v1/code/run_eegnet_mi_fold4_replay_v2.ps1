...
$log = Join-Path $runtime 'scheduled_replay_EEGNet_OpenBMI_MI_f4_v2.log'
...
try {
  ...
  "REPLAY_EEGNET_MI_F4_START free_gb=$([Math]::Round($freeGb,2)) gpu_mib=$gpuMiB utc=$([DateTime]::UtcNow.ToString('o'))" | Out-File -LiteralPath $log -Encoding utf8
  $python = 'E:\Anaconda\envs\persist_stable_251\python.exe'
  $command = '"' + $python + '" -u "' + $entry + '" --task OpenBMI_MI --fold 4 >> "' + $log + '" 2>&1'
  & cmd.exe /d /c $command
  ...
  "REPLAY_EEGNET_MI_F4_END exit=0 utc=$([DateTime]::UtcNow.ToString('o'))" | Out-File -LiteralPath $log -Append -Encoding utf8
  exit 0
} catch {
  if (Test-Path -LiteralPath $log) { "REPLAY_EEGNET_MI_F4_EXCEPTION $_" | Out-File -LiteralPath $log -Append -Encoding utf8 }
  exit 1
}
