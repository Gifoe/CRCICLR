<# Wait for fold3 mediation, then continuously finish exact directional v4. #>
...
$taskName = 'PERSIST_EEG_PC_MECHANISM_CELL_EEGNET_MI_F3_DIRECTIONAL_QUEUE_V4'
...
$log = Join-Path $runtime 'scheduled_directional_cell_EEGNet_OpenBMI_MI_f3_queue_v4.log'
...
function Confirm-Inputs {
  ...
  if (Test-Path -LiteralPath $mediation) {
    ...
    if ($m.status -ne 'MEDIATION_PHASE_COMPLETE_PENDING_OTHER_REQUIRED_ANALYSES' -or
        $m.model -ne 'EEGNet' -or $m.task -ne 'OpenBMI_MI' -or $m.fold -ne 3 -or
        $m.stage_count -ne 4 -or $m.final_heldout_accessed -ne $false) {
      throw 'fold3 mediation manifest invalid'
    }
  }
}
...
function Progress-Count {
  ...
  foreach ($file in (Get-ChildItem -LiteralPath $parts -Filter '*.partial.json' -File)) {
    ...
    if ($partial.status -ne 'PARTIAL_RESOURCE_SAFE' -or $partial.final_heldout_accessed -ne $false) {
      throw "invalid directional partial: $($file.Name)"
    }
    $count += @($partial.random_controls).Count
  }
  $count += 100 * @(Get-ChildItem -LiteralPath $parts -Filter '*.json' -File |
    Where-Object { $_.Name -notlike '*.partial.json' }).Count
  return $count
}
...
try {
  ...
  if ((Test-Path -LiteralPath $manifest) -or (Test-Path -LiteralPath $failure)) {
    throw 'directional terminal evidence already exists'
  }
  Confirm-Inputs
  "DIRECTIONAL_F3_QUEUE_START utc=$([DateTime]::UtcNow.ToString('o'))" | Out-File -LiteralPath $log -Encoding utf8
  for ($attempt = 1; $attempt -le 100; $attempt++) {
    ...
    while ($true) {
      ...
      if ((Test-Path -LiteralPath $mediation) -and $resource.FreeGb -ge 40 -and
          $resource.GpuMiB -ge 16000 -and -not $blocked) { break }
      $waits++
      if ($waits -eq 1 -or $waits % 10 -eq 0) {
        "DIRECTIONAL_F3_WAIT attempt=$attempt mediation=$([bool](Test-Path -LiteralPath $mediation)) free_gb=$([Math]::Round($resource.FreeGb,2)) gpu_mib=$($resource.GpuMiB) blocked=$blocked utc=$([DateTime]::UtcNow.ToString('o'))" |
    ...
    $stdout = Join-Path $runtime "scheduled_directional_cell_EEGNet_OpenBMI_MI_f3_queue_v4_attempt${attempt}.stdout.log"
    $stderr = Join-Path $runtime "scheduled_directional_cell_EEGNet_OpenBMI_MI_f3_queue_v4_attempt${attempt}.stderr.log"
    if ((Test-Path -LiteralPath $stdout) -or (Test-Path -LiteralPath $stderr)) { throw 'attempt log already exists' }
    "DIRECTIONAL_F3_ATTEMPT_START attempt=$attempt free_gb=$([Math]::Round($resource.FreeGb,2)) gpu_mib=$($resource.GpuMiB) utc=$([DateTime]::UtcNow.ToString('o'))" |
      Out-File -LiteralPath $log -Append -Encoding utf8
    $child = Start-Process -FilePath $python -ArgumentList @('-u', $entry, '--model', 'EEGNet', '--task', 'OpenBMI_MI', '--fold', '3') `
    ...
    while ($true) {
      ...
      if ($resource.FreeGb -lt 16 -or $resource.GpuMiB -lt 1000) {
        ...
        if ($owned -and $owned.Name -match '^python(\.exe)?$' -and
            $owned.CommandLine -match 'run_directional_cell_v4.py' -and
            $owned.CommandLine -match 'OpenBMI_MI' -and $owned.CommandLine -match '--fold 3') {
          Stop-Process -Id $child.Id -Force -ErrorAction Stop
          $safetyStop = $true
          "DIRECTIONAL_F3_SAFETY_STOP attempt=$attempt pid=$($child.Id) free_gb=$([Math]::Round($resource.FreeGb,2)) gpu_mib=$($resource.GpuMiB) utc=$([DateTime]::UtcNow.ToString('o'))" |
            Out-File -LiteralPath $log -Append -Encoding utf8
          break
        }
      }
    ...
    if (Test-Path -LiteralPath $manifest) {
      ...
      if ($done.status -ne 'DIRECTIONAL_PHASE_COMPLETE_PENDING_OTHER_REQUIRED_ANALYSES' -or
          $done.model -ne 'EEGNet' -or $done.task -ne 'OpenBMI_MI' -or $done.fold -ne 3 -or
          $done.stage_count -ne 5 -or $done.random_draws_per_stage -ne 20 -or
          $done.final_heldout_accessed -ne $false) { throw 'directional terminal manifest invalid' }
      "DIRECTIONAL_F3_COMPLETE attempt=$attempt utc=$([DateTime]::UtcNow.ToString('o'))" |
        Out-File -LiteralPath $log -Append -Encoding utf8
      exit 0
    }
    if (Test-Path -LiteralPath $failure) { throw 'directional phase failed closed; do not retry' }
    $after = Progress-Count
    if (-not $safetyStop -and $after -le $before) {
      throw "directional process made no atomic progress: before=$before after=$after"
    }
    "DIRECTIONAL_F3_RECYCLE attempt=$attempt safety_stop=$safetyStop progress_before=$before progress_after=$after utc=$([DateTime]::UtcNow.ToString('o'))" |
      Out-File -LiteralPath $log -Append -Encoding utf8
  }
  throw 'directional phase exceeded 100 attempts'
} catch {
  if (Test-Path -LiteralPath $log) {
    "DIRECTIONAL_F3_QUEUE_EXCEPTION $_ utc=$([DateTime]::UtcNow.ToString('o'))" |
      Out-File -LiteralPath $log -Append -Encoding utf8
  }
  exit 1
}
