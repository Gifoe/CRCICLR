"""Checkpoint-native TRAIN-only PERSIST discovery on an EEGNet replica epoch v2.
...
def main() -> None:
    ...
    result = cell / "checkpoint_native_pt_v2" / f"epoch_{epoch:03d}.json"
    ...
    try:
        ...
        if (search_data["split_sha256"] != record["split_sha256"] or
            ...
            raise RuntimeError("frozen SEARCH data identity mismatch")
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        model = S.build_model("EEGNet", dataset=search_data["dataset"],
                              channels=int(search_data["channels"]),
                              samples=int(search_data["samples"]), classes=int(search_data["classes"]),
        ...
        C.write_json(result, {
            ...
            "correction_provenance": "V2_SOURCE_RECORD_DIMENSIONS_KEY_FIX_V1_FAIL_PRESERVED",
        ...
        print("NATIVE_PT_V2_COMPLETE", task, fold, epoch, "rank", len(protected), flush=True)
