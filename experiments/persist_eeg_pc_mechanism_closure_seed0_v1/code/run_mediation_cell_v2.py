"""Production mediation phase v2; float64 P+C arithmetic after v1 failure.
...
from mediation_core_v5 import LONG_RANGE_STAGES, adjacent_mediation, long_range_mediation, verify_adjacent_forward  # noqa: E402
...
def main() -> None:
    ...
    manifest = cell_dir / "MEDIATION_V2.json"
    failure = cell_dir / "MEDIATION_V2_FAIL_CLOSED.json"
    if manifest.exists() or failure.exists():
        raise RuntimeError("mediation cell phase already has terminal evidence; no overwrite")
    try:
        if (model, task, fold) == ("EEGNet", "OpenBMI_MI", 0):
            original_failure = cell_dir / "MEDIATION_V1_FAIL_CLOSED.json"
            numerical_audit = EXP / "probe" / "EEGNet_OpenBMI_MI_fold0_reconstruction_failure_diagnostic_v2.json"
            if not original_failure.exists() or not numerical_audit.exists():
                raise RuntimeError("v1 failure and all-pair precision audit must remain available")
            audit = json.loads(numerical_audit.read_text(encoding="utf-8"))
            if (audit.get("pair_count") != 832 or audit.get("above_threshold_1e_minus_5") != 1
                    or audit.get("full_double_projection_reconstruction_max_abs", 1.0) >= 1e-5
                    or audit.get("final_heldout_accessed") is not False):
                raise RuntimeError("all-pair numerical audit does not support float64 correction")
        ...
        for stage in runner.names[:-1]:
            destination = cell_dir / "mediation_v2" / f"{stage}.json"
            ...
            C.write_json(destination, {
                "status": "STAGE_COMPLETE", "schema": "PC_MECHANISM_MEDIATION_STAGE_V2",
                ...
                "mediation_core_sha256": C.digest(EXP / "code" / "mediation_core_v5.py"),
        ...
        C.write_json(manifest, {
            ...
            "upstream_implementation_sha256": ORIGINAL_IMPL_SHA,
            "mediation_core_sha256": C.digest(EXP / "code" / "mediation_core_v5.py"),
            "v1_fail_closed_evidence": str(cell_dir / "MEDIATION_V1_FAIL_CLOSED.json"),
            "numerical_correction": "FLOAT64_PROJECTION_AND_REASSEMBLY_MODEL_FORWARD_FLOAT32",
            "implementation_sha256": C.digest(Path(__file__)), "final_heldout_accessed": False,
        })
