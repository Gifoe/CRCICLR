...
$name = 'PERSIST_EEG_PC_MECHANISM_CELL_EEGNET_MI_F1_REPLAY_V1'
$script = 'D:\nips-temp\TotalP\P1\CRCICLR_PERSIST_INCREMENTAL_VALUE_V1\experiments\persist_eeg_pc_mechanism_closure_seed0_v1\code\run_eegnet_mi_fold1_replay_v1.ps1'
if (Get-ScheduledTask -TaskName $name -ErrorAction SilentlyContinue) { throw 'task already exists' }
if ((Get-FileHash -LiteralPath $script -Algorithm SHA256).Hash.ToLowerInvariant() -ne '62b6e708798be3abb807c8164fbc692a4721d4febc97f8387c26fa1a448c661a') { throw 'script SHA mismatch' }
