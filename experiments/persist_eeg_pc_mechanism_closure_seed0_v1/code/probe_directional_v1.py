"""TRAIN-only engineering probe for native directional finite differences."""
...
from directional_core_v1 import fit_directions, mixed_difference  # noqa: E402
from run_mediation_cell_v2 import verified_context  # noqa: E402
from sampling_core import trial_view  # noqa: E402


def main() -> None:
    result = EXP / "probe" / "EEGNet_OpenBMI_MI_fold0_directional_probe_v1.json"
    if result.exists():
    ...
    for stage in (runner.names[0], runner.names[-1]):
        ...
        effect = mixed_difference(runner, stage, tuple(centroid["shapes"][stage]),
                                  x, labels, logits, axes, (0, 0), epsilon=0.5)
    ...
    C.write_json(result, {"status": "ENGINEERING_PROBE_PASSED", "data_scope": "TRAIN_ONLY",
    ...
    print("DIRECTIONAL_PROBE_PASSED", len(checks), flush=True)
