"""TRAIN-ranked P×C directional and first-20 random-control cell phase.
...
from directional_core_v1 import fit_directions, frozen_top_pairs, mixed_difference  # noqa: E402
...
def main() -> None:
    ...
    manifest = cell_dir / "DIRECTIONAL_V1.json"
    failure = cell_dir / "DIRECTIONAL_V1_FAIL_CLOSED.json"
    ...
    try:
        ...
        for stage in runner.names:
            destination = cell_dir / "directional_v1" / f"{stage}.json"
            ...
            outer_matrix, outer_effects, outer_id = matrix_and_pairs(runner, stage, shape,
            ...
            for draw, subset in enumerate(randoms):
                ...
                controls.append({
                    ...
                    "final_subset_sha256": hashlib.sha256(np.ascontiguousarray(subset).tobytes()).hexdigest(),
                ...
                print("DIRECTIONAL_RANDOM_DONE", model, task, fold, stage, draw + 1, flush=True)
            C.write_json(destination, {
                "status": "STAGE_COMPLETE", "schema": "PC_MECHANISM_DIRECTIONAL_STAGE_V1",
