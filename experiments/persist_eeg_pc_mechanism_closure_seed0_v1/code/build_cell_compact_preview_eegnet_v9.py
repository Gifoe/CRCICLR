"""Audit and render a compact preview for one frozen Phase 2 cell.
...
def main() -> None:
    ...
    parser.add_argument("--subject-v2-sha", required=True)
    args = parser.parse_args()
    TASK, FOLD = args.task, args.fold
    subject_v2_sha = args.subject_v2_sha.lower()
    cell = RUNTIME / "cells" / MODEL.lower() / TASK.lower() / f"fold{FOLD}_seed0"
    output = RUNTIME / "compact_preview" / f"eegnet_{TASK.lower()}_fold{FOLD}_seed0_v9"
    if output.exists():
        raise RuntimeError("compact preview path already has evidence")
    ...
    try:
        ...
        endpoint = checked(cell / "REPLICA_ENDPOINT_AUDIT_V1.json",
                           ...
                           gate_sha=gate_sha, protocol_sha=protocol_sha)
        backtrace = checked(cell / "FINAL_P_BACKTRACE_V2.json",
        ...
        if not (len(med["stage_sha256"]) == 4 and len(dire["stage_sha256"]) == 5):
            raise RuntimeError("frozen mediation/directional stage count mismatch")
        ...
        schedule_epochs = [int(row["epoch"]) for row in endpoint["schedule"]]
        ...
        if (set(epochs) != set(backs) or len(schedule_epochs) != len(set(schedule_epochs)) or
                schedule_epochs != [int(value) for value in backtrace["checkpoint_schedule_epochs"]]):
        ...
        for epoch in schedule_epochs:
            ...
            native = checked(p, "NATIVE_CHECKPOINT_PT_COMPLETE_PENDING_BACKTRACE_AND_MECHANISM",
                             gate_sha=gate_sha, protocol_sha=protocol_sha)
            p = cell / "checkpoint_mechanism_v2" / f"epoch_{epoch:03d}.json"
            ck = checked(p, "CHECKPOINT_MECHANISM_COMPLETE_REPLICA_ONLY",
                         gate_sha=gate_sha, protocol_sha=protocol_sha)
            pair_coverage = ck["pair_coverage"]
            expected_pair_count = sum(
                int(row["selected_recipients"]) * len(row["other_classes"])
            ...
            if (native["trajectory_provenance"] != "RETRAINED_REPLICA_TRAJECTORY" or
                    ...
                    ck["final_p_backtrace_audit_sha256"] != C.digest(cell / "FINAL_P_BACKTRACE_V2.json") or
                    ck["pair_count"] != expected_pair_count or len(ck["subject_session_mechanism"]) != 52 or
                    ck["subject_session_count"] != 52 or any(int(row["selected_recipients"]) > 8 for row in pair_coverage)):
            ...
            for row in backs[epoch]["subject_session_recovery"]:
                backtrace_rows.append({"model": MODEL, "task": TASK, "fold": FOLD,
                                       "epoch": epoch, "trajectory_provenance": "RETRAINED_REPLICA_TRAJECTORY",
                                       **row})
        if len(mechanism) != len(schedule_epochs) * 52:
        ...
        report = (
            ...
            f"EEGNet / {TASK} / seed0 / fold{FOLD} only. All representations and directions "
        ...
        C.write_json(output / "PROVENANCE.json", {
            "status": "CELL_COMPACT_PREVIEW_COMPLETE_NOT_20_CELL_RESULT",
            ...
            "source_backtrace_sha256": C.digest(cell / "FINAL_P_BACKTRACE_V2.json"),
