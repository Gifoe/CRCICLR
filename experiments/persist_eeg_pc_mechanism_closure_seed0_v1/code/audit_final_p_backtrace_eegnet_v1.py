"""TRAIN-only cross-subject recovery of the frozen historical final Protected code.

Replica checkpoints are not the historical training trajectory. A primal ridge
solve is algebraically identical to the frozen pathway's linear kernel ridge,
but scales with feature width rather than the number of capped trials.
"""
...
def main() -> None:
    ...
    result = cell / "FINAL_P_BACKTRACE_V1.json"
    failure = cell / "FINAL_P_BACKTRACE_V1.FAIL_CLOSED.json"
    if result.exists() or failure.exists():
        raise RuntimeError("final-P backtrace already has terminal evidence")
    try:
        ...
        if (replay.get("status") != "REPLAY_COMPLETE" or
                ...
                endpoint.get("final_heldout_accessed") is not False):
        ...
        schedule_epochs = [int(row["epoch"]) for row in schedule_rows]
        if len(schedule_epochs) != 6 or len(schedule) != 6:
            raise RuntimeError("endpoint-derived checkpoint schedule invalid")
        output = []
        ...
        print("FINAL_P_BACKTRACE_V1_COMPLETE", task, fold, len(output), flush=True)
