...
$entry=Join-Path $exp 'code\audit_native_pt_eegnet_v1.py'
$result=Join-Path $cell 'checkpoint_native_pt_v1\epoch_006.json'
$failure=Join-Path $cell 'checkpoint_native_pt_v1\epoch_006.FAIL_CLOSED.json'
$endpoint=Join-Path $cell 'REPLICA_ENDPOINT_AUDIT_V1.json'
$log=Join-Path $runtime 'scheduled_native_pt_EEGNet_OpenBMI_MI_f0_e006_v1.log'
...
try {
  ...
  if((Get-FileHash -LiteralPath $entry -Algorithm SHA256).Hash.ToLowerInvariant() -ne '06e68c10b58370d1571691ae2c52231fa3f5efec29e5acd2bebc107fad9268b2') {throw 'native P_t source SHA mismatch'}
  ...
  "NATIVE_PT_EEGNET_MI_F0_E006_START utc=$([DateTime]::UtcNow.ToString('o'))" | Out-File -LiteralPath $log -Encoding utf8
  ...
  "NATIVE_PT_EEGNET_MI_F0_E006_END exit=0 utc=$([DateTime]::UtcNow.ToString('o'))" | Out-File -LiteralPath $log -Append -Encoding utf8
  exit 0
} catch {
  "NATIVE_PT_EEGNET_MI_F0_E006_EXCEPTION $_" | Out-File -LiteralPath $log -Append -Encoding utf8
  exit 1
}
