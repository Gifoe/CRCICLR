<# Frozen TRAIN-fit subject/session signature; outer evaluation is descriptive. #>
...
$entry=Join-Path $exp 'code\audit_subject_session_eegnet_mi_f0_v1.py'
$result=Join-Path $cell 'SUBJECT_SESSION_AUDIT_V1.json'
$failure=Join-Path $cell 'SUBJECT_SESSION_AUDIT_V1.FAIL_CLOSED.json'
$log=Join-Path $runtime 'scheduled_subject_session_EEGNet_OpenBMI_MI_f0_v1.log'
...
try {
  if((Test-Path -LiteralPath $log) -or (Test-Path -LiteralPath $result) -or (Test-Path -LiteralPath $failure)) {throw 'subject/session terminal evidence exists'}
  if((Get-FileHash -LiteralPath $entry -Algorithm SHA256).Hash.ToLowerInvariant() -ne '3109bd2a4c93f0681d15f80030fe612520228ecfa2f7adc127838c1cd58f57ee') {throw 'subject/session source SHA mismatch'}
  ...
  "SUBJECT_SESSION_EEGNET_MI_F0_V1_START utc=$([DateTime]::UtcNow.ToString('o'))" | Out-File -LiteralPath $log -Encoding utf8
  $python='E:\Anaconda\envs\persist_stable_251\python.exe'
  $command='"'+$python+'" -u "'+$entry+'" >> "'+$log+'" 2>&1'
  & cmd.exe /d /c $command
  if($LASTEXITCODE -ne 0) {throw "subject/session audit exited $LASTEXITCODE"}
  $row=Get-Content -LiteralPath $result -Raw|ConvertFrom-Json
  if($row.status -ne 'SUBJECT_SESSION_AUDIT_COMPLETE_POST_OUTCOME_DESCRIPTIVE' -or $row.final_heldout_accessed -ne $false -or @($row.subject_session_signature_rows).Count -ne 68) {throw 'subject/session result invalid'}
  "SUBJECT_SESSION_EEGNET_MI_F0_V1_END exit=0 utc=$([DateTime]::UtcNow.ToString('o'))" | Out-File -LiteralPath $log -Append -Encoding utf8
  exit 0
} catch {
  "SUBJECT_SESSION_EEGNET_MI_F0_V1_EXCEPTION $_" | Out-File -LiteralPath $log -Append -Encoding utf8
  exit 1
}
