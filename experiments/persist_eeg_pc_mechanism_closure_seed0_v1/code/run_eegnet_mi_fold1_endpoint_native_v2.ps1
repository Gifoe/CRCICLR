<# Resume fold1 native P_t audits from the immutable endpoint-derived schedule. #>
...
$mainLog = Join-Path $runtime 'scheduled_endpoint_native_EEGNet_OpenBMI_MI_f3_v3.log'
...
try {
  if (Test-Path -LiteralPath $mainLog) { throw 'v2 pipeline log already exists' }
  ...
  while (-not (Test-Path -LiteralPath $replayPath)) {
    if (Test-Path -LiteralPath (Join-Path $cell 'replay_v2\FAIL_CLOSED.json')) { throw 'replay failed closed' }
  ...
  if (-not (Test-Path -LiteralPath $endpointPath)) {
    ...
    $endpointLog = Join-Path $runtime 'scheduled_replica_endpoint_audit_EEGNet_OpenBMI_MI_f3_v1.log'
    ...
    $cmd = '"' + $python + '" -u "' + $endpointEntry + '" --task OpenBMI_MI --fold 3 >> "' + $endpointLog + '" 2>&1'
    & cmd.exe /d /c $cmd
  ...
  if (($epochs | Select-Object -Unique).Count -ne 6 -or $epochs[0] -ne 6 -or $epochs[1] -ne 15 -or $epochs[2] -ne 30 -or $epochs[3] -ne 45 -or $epochs[-1] -ne 60) { throw 'endpoint-derived schedule shape invalid' }
  "ENDPOINT_NATIVE_F3_V3_START schedule=$($epochs -join ',') utc=$([DateTime]::UtcNow.ToString('o'))" | Out-File -LiteralPath $mainLog -Encoding utf8
  ...
  foreach ($epoch in $epochs) {
    ...
    if (-not (Test-Path -LiteralPath $result)) {
      ...
      $phaseLog = Join-Path $runtime "scheduled_native_pt_EEGNet_OpenBMI_MI_f3_e${tag}_v3.log"
      if (Test-Path -LiteralPath $phaseLog) { throw "native P_t log already exists at legal epoch $epoch" }
  ...
  "ENDPOINT_NATIVE_F3_V3_COMPLETE utc=$([DateTime]::UtcNow.ToString('o'))" | Out-File -LiteralPath $mainLog -Append -Encoding utf8
  exit 0
} catch {
  if (Test-Path -LiteralPath $mainLog) { "ENDPOINT_NATIVE_F3_V3_EXCEPTION $_ utc=$([DateTime]::UtcNow.ToString('o'))" | Out-File -LiteralPath $mainLog -Append -Encoding utf8 }
  exit 1
}
