...
$result=Join-Path $cell 'checkpoint_native_pt_v3\epoch_006.json'
$failure=Join-Path $cell 'checkpoint_native_pt_v3\epoch_006.FAIL_CLOSED.json'
$endpoint=Join-Path $cell 'REPLICA_ENDPOINT_AUDIT_V1.json'
$log=Join-Path $runtime 'scheduled_native_pt_EEGNet_OpenBMI_MI_f0_e006_v3.log'
...
try {
  ...
  "NATIVE_PT_EEGNET_MI_F0_E006_V3_START utc=$([DateTime]::UtcNow.ToString('o'))" | Out-File -LiteralPath $log -Encoding utf8
  ...
  "NATIVE_PT_EEGNET_MI_F0_E006_V3_END exit=0 utc=$([DateTime]::UtcNow.ToString('o'))" | Out-File -LiteralPath $log -Append -Encoding utf8
  exit 0
} catch {
  "NATIVE_PT_EEGNET_MI_F0_E006_V3_EXCEPTION $_" | Out-File -LiteralPath $log -Append -Encoding utf8
  exit 1
}
