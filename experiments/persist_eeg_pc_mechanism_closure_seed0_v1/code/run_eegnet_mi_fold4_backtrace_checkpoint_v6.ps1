<# Corrected V6: accept the frozen native-P_t status emitted by the audited V6 producer. #>
$ErrorActionPreference='Stop'
$repo='D:\nips-temp\TotalP\P1\CRCICLR_PERSIST_INCREMENTAL_VALUE_V1';$exp=Join-Path $repo 'experiments\persist_eeg_pc_mechanism_closure_seed0_v1';$rt='D:\nips-temp\TotalP\P1\pc_mechanism_closure_runtime';$cell=Join-Path $rt 'cells\eegnet\openbmi_mi\fold4_seed0';$py='E:\Anaconda\envs\persist_stable_251\python.exe';$back=Join-Path $exp 'code\audit_final_p_backtrace_eegnet_v2.py';$mech=Join-Path $exp 'code\audit_checkpoint_mechanism_eegnet_v2.py';$log=Join-Path $rt 'scheduled_backtrace_checkpoint_EEGNet_OpenBMI_MI_f4_v6.log'
...
try{
 if(Test-Path $log){throw 'V6 log already exists'}
 ...
 'BACKTRACE_CHECKPOINT_F4_V6_START'|Out-File $log -Encoding utf8
 ...
 foreach($e in $epochs){$p=Join-Path $cell ("checkpoint_native_pt_v3\epoch_{0:d3}.json"-f$e);if(-not(Test-Path $p)){throw "native P_t missing $e"};$j=Get-Content $p -Raw|ConvertFrom-Json;if($j.status-ne'NATIVE_CHECKPOINT_PT_COMPLETE_PENDING_BACKTRACE_AND_MECHANISM'-or$j.final_heldout_accessed-ne$false){throw "native P_t invalid $e"}}
 $out=Join-Path $cell 'FINAL_P_BACKTRACE_V1.json';if(-not(Test-Path $out)){Run 'BACKTRACE' ('"'+$back+'" --task OpenBMI_MI --fold 4') 'scheduled_final_p_backtrace_EEGNet_OpenBMI_MI_f4_v6.log'};$j=Get-Content $out -Raw|ConvertFrom-Json;if($j.status-ne'FINAL_P_BACKTRACE_COMPLETE_REPLICA_ONLY'-or$j.final_heldout_accessed-ne$false){throw 'backtrace invalid'}
 foreach($e in $epochs){$tag='{0:d3}'-f$e;$out=Join-Path $cell "checkpoint_mechanism_v1\epoch_$tag.json";if(-not(Test-Path $out)){Run "MECH_$tag" ('"'+$mech+'" --task OpenBMI_MI --fold 4 --epoch '+$e) "scheduled_checkpoint_mechanism_EEGNet_OpenBMI_MI_f4_e${tag}_v6.log"};$j=Get-Content $out -Raw|ConvertFrom-Json;if($j.status-ne'CHECKPOINT_MECHANISM_COMPLETE_REPLICA_ONLY'-or$j.final_heldout_accessed-ne$false){throw "mechanism invalid $e"}}
 'BACKTRACE_CHECKPOINT_F4_V6_COMPLETE'|Out-File $log -Append -Encoding utf8;exit 0
}catch{"BACKTRACE_CHECKPOINT_F4_V6_EXCEPTION $_"|Out-File $log -Append -Encoding utf8;exit 1}
