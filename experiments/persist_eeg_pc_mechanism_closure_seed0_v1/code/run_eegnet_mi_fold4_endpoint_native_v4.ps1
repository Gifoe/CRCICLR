...
$mainLog = Join-Path $runtime 'scheduled_endpoint_native_EEGNet_OpenBMI_MI_f4_v4.log'
...
try {
  ...
  if (-not (Test-Path -LiteralPath $endpointPath)) {
    ...
    $endpointLog = Join-Path $runtime 'scheduled_replica_endpoint_audit_EEGNet_OpenBMI_MI_f4_v1.log'
    ...
    $cmd = '"' + $python + '" -u "' + $endpointEntry + '" --task OpenBMI_MI --fold 4 >> "' + $endpointLog + '" 2>&1'
    & cmd.exe /d /c $cmd
  ...
  if (($epochs | Select-Object -Unique).Count -ne 6 -or @($required | Where-Object { $_ -notin $epochs }).Count -ne 0) { throw 'endpoint-derived schedule set invalid' }
  "ENDPOINT_NATIVE_F4_V4_START schedule=$($epochs -join ',') utc=$([DateTime]::UtcNow.ToString('o'))" | Out-File -LiteralPath $mainLog -Encoding utf8
  ...
  foreach ($epoch in $epochs) {
    ...
    if (-not (Test-Path -LiteralPath $result)) {
      ...
      $phaseLog = Join-Path $runtime "scheduled_native_pt_EEGNet_OpenBMI_MI_f4_e${tag}_v4.log"
      if (Test-Path -LiteralPath $phaseLog) { throw "native P_t log already exists at legal epoch $epoch" }
      $cmd = '"' + $python + '" -u "' + $nativeEntry + '" --task OpenBMI_MI --fold 4 --epoch ' + $epoch + ' >> "' + $phaseLog + '" 2>&1'
      & cmd.exe /d /c $cmd
  ...
  "ENDPOINT_NATIVE_F4_V4_COMPLETE utc=$([DateTime]::UtcNow.ToString('o'))" | Out-File -LiteralPath $mainLog -Append -Encoding utf8
  exit 0
} catch {
  if (Test-Path -LiteralPath $mainLog) { "ENDPOINT_NATIVE_F4_V4_EXCEPTION $_ utc=$([DateTime]::UtcNow.ToString('o'))" | Out-File -LiteralPath $mainLog -Append -Encoding utf8 }
  exit 1
}
