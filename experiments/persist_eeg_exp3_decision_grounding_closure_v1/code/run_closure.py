...
def finalize(frame: pd.DataFrame, comp: dict[str, Any], pred: pd.DataFrame, summary: pd.DataFrame, deltas: dict[str, Any], decision: dict[str, Any], audit: dict[str, Any]) -> dict[str, Any]:
    ...
    lines = [
        ...
        f"11. Adding decision dependence to baseline: ΔRMSE(M0−MD)={tests['A_MD_vs_M0']['mean']:.8f}; 95% CI={tests['A_MD_vs_M0']['ci95']}; exact run sign-flip p={tests['A_MD_vs_M0']['sign_flip_p_exact']:.8f}.",
        f"12. MD outperforms MI: **{md_vs_mi_pass}**; ΔRMSE(MI−MD)={tests['B_MD_vs_MI']['mean']:.8f}; 95% CI={tests['B_MD_vs_MI']['ci95']}; exact p={tests['B_MD_vs_MI']['sign_flip_p_exact']:.8f}.",
        f"13. Runs favoring MD over MI: {tests['B_MD_vs_MI']['positive_runs']}/6.",
        f"14. Run-cluster CI for MD−MI comparison: {tests['B_MD_vs_MI']['ci95']}.",
        f"15. Exact sign-flip p for MD−MI: {tests['B_MD_vs_MI']['sign_flip_p_exact']:.8f}.",
        f"16. Identity after decision (MD vs MID): ΔRMSE(MD−MID)={tests['C_identity_after_D']['mean']:.8f}; this is descriptive and is not interpreted as proof that identity contributes zero.",
