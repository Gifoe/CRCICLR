...
EXP = REPO / "experiments/persist_eeg_litebn_tfformer_pswa_v1"
OUT = EXP / "outputs/pswa_v1"
PROTOCOL = EXP / "protocol"
RUNTIME = Path("/root/rivermind-data/litebn_tfformer_pswa_v1_runtime")
SESSION_CACHE = RUNTIME / "session_embeddings"
RUN_CACHE = RUNTIME / "run_results"
PEEH_EXP = REPO / "experiments/persist_eeg_litebn_tfformer_peeh_v1"
PEEH_OUT = PEEH_EXP / "outputs/peeh_v1"
PEEH_RUNTIME = Path("/root/rivermind-data/litebn_tfformer_peeh_v1_runtime")
PEEH_EMBED = PEEH_RUNTIME / "embeddings"
PEEH_RUN = PEEH_RUNTIME / "run_results"
PEEH_CODE = PEEH_EXP / "code/run_peeh.py"
...
def audit_artifacts(seeds: Sequence[int]) -> list[dict[str, Any]]:
    required = [PEEH_CODE, PEEH_OUT / "RUN_LEVEL_PEEH.csv", PEEH_OUT / "PROTECTED_BLOCK_SELECTION.csv",
                PEEH_OUT / "TASK_LEVEL_PEEH.csv", PEEH_EXP / "protocol/PROTOCOL_LOCK.json"]
...
def aggregate(results: Sequence[Mapping[str, Any]], total_expected_runs: int):
    ...
    paired = []
    for task in TASKS:
        a = subject[(subject.task == task) & (subject.model == "LiteBN")][["subject_id","PSWA_pp"]].rename(columns={"PSWA_pp":"LiteBN_PSWA_pp"})
        b = subject[(subject.task == task) & (subject.model == "TFFormer")][["subject_id","PSWA_pp"]].rename(columns={"PSWA_pp":"TFFormer_PSWA_pp"})
        p = a.merge(b, on="subject_id", validate="one_to_one"); delta = (p.TFFormer_PSWA_pp-p.LiteBN_PSWA_pp).to_numpy(float)
        lo, hi = bootstrap(delta, stable_seed("PSWA-paired-bootstrap", task))
        paired.append({"task": task, "biological_subjects": len(p), "LiteBN_PSWA_mean_pp": float(p.LiteBN_PSWA_pp.mean()),
                       "TFFormer_PSWA_mean_pp": float(p.TFFormer_PSWA_pp.mean()), "TFFormer_minus_LiteBN_PSWA_mean_pp": float(delta.mean()),
...
def report(task: pd.DataFrame, paired: pd.DataFrame) -> str:
    lines = ["# Frozen LiteBN / TFFormer Protected-Subspace Worst-Session Advantage", "",
    ...
    lines += ["", "## Paired LiteBN vs TFFormer", "", "| Task | LiteBN PSWA | TFFormer PSWA | TFFormer - LiteBN [95% CI] |", "|---|---:|---:|---:|"]
    for r in paired.itertuples(index=False): lines.append(f"| {r.task} | {r.LiteBN_PSWA_mean_pp:.2f} | {r.TFFormer_PSWA_mean_pp:.2f} | {r.TFFormer_minus_LiteBN_PSWA_mean_pp:.2f} [{r.delta_CI95_low_pp:.2f}, {r.delta_CI95_high_pp:.2f}] |")
...
def analyze(seeds: Sequence[int], recovery_rows: Sequence[Mapping[str, Any]]) -> None:
    ...
    atomic_csv(OUT / "LITEBN_TFFORMER_PAIRED_PSWA.csv", paired)
