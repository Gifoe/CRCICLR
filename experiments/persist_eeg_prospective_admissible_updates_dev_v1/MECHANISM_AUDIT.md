...
`results/GRADIENT_CONFLICT_SUMMARY.csv` reports the per-candidate conflict rate, joint activation, cosine and correction size (the raw per-step log remains on the server and is intentionally not committed under the compact-artifact policy). `results/PROSPECTIVE_HARM_LOG.csv` measures the fixed pseudo-future diagnostic B loss every 20 steps and is never used in updates or selection.
