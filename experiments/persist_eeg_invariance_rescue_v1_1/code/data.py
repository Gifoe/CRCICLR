...
class DevelopmentSplit:
    ...
    def train_subjects(self) -> tuple[str, ...]:
        """All 34 legal development-train subjects used for V1.1 refits."""
        return self.original_train_subjects
...
def load_development_split(fold: int) -> DevelopmentSplit:
    ...
    # V1.1 performs all selection in explicit inner subject CV and then
    # refits every final model on all 34 frozen development-train subjects.
    calibration = tuple()
    model_fit = tuple(original_train)
    if len(model_fit) != int(config["train_subject_count"]):
        raise RuntimeError("V1.1 development train subject count mismatch")
...
def normalizer(fold: int, manifest: pd.DataFrame, subjects: Sequence[str]) -> tuple[np.ndarray, np.ndarray, Path]:
    ...
    # Inner subject-CV uses different legal subsets.  Keeping the subject hash
    # in the cache name prevents an inner fold from silently reusing (or
    # overwriting) the final 34-subject normalizer.
    suffix = "FULL" if set(map(str, subjects)) == set(map(str, load_development_split(fold).original_train_subjects)) else expected_hash[:12]
    path = CACHE / f"NORMALIZER_FOLD_{fold}_{suffix}.npz"
    if path.exists():
