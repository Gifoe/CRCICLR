...
def determine_eligibility(identity: pd.DataFrame, task: pd.DataFrame, functional: pd.DataFrame, assignments: pd.DataFrame) -> dict[str, Any]:
    ...
    for family in families:
        ...
        run_count = int(idf[["fold", "seed"]].drop_duplicates().shape[0]) if len(idf) else 0
        task_models = {name: float(idf[name].mean()) for name in ("T_anchor_BA", "T_replica_BA", "invariant_BA", "T_anchor_ID", "T_replica_ID", "invariant_ID") if name in idf.columns and len(idf)}
        result[family] = {"runs": run_count, "subject_rows": int(len(idf)), "valid_assignment_runs": int(len(valid_runs)), "I1": i1, "I1_certified": i1_cert, "I2": i2, "I2_certified": i2_cert, "I3": i3, "I3_certified": i3_cert, "measurement_invalid": measurement_invalid, "status": status, "eligible": bool(status == "ELIGIBLE_PROTECTED_LOSS"), "rescue_allowed": rescue_allowed, "task_models": task_models, "identity": id_summary, "task_harm": ba_summary, "protected_loss": lp_summary, "matched_nonprotected_loss": ln_summary, "selective_loss": spl_summary, "p_values": {"raw": raw_p}, "outer_test_used": False, "outer_membership_enumerated": False}
...
def finalize() -> dict[str, Any]:
    ...
    for family, row in eligibility["families"].items(): report.append(f"- {family}: T_anchor/T_replica/I BA={row['task_models'].get('T_anchor_BA')}/{row['task_models'].get('T_replica_BA')}/{row['task_models'].get('invariant_BA')}; I1={row['I1']} (mean ΔID={row['identity']['mean']}, CI={row['identity']['ci95']}), I2={row['I2']} (mean L_P={row['protected_loss']['mean']}, mean L_N={row['matched_nonprotected_loss']['mean']}, mean SPL={row['selective_loss']['mean']}, CI={row['selective_loss']['ci95']}), I3={row['I3']} (mean ΔBA={row['task_harm']['mean']}, CI={row['task_harm']['ci95']}), status={row['status']}.")
