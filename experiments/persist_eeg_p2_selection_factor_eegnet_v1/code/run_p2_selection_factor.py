"""Frozen EEGNet P2 selection-factor ablation.
...
def peeh_path(task: str, fold: int) -> Path:
    return PEEH_RUNTIME / "cells" / "eegnet" / task.lower() / f"fold{fold}_seed0.json"


def pswa_path(task: str, fold: int) -> Path:
    return PSWA_RUNTIME / "cells" / "eegnet" / task.lower() / f"fold{fold}_seed0.json"
...
def reconstruct_persistence(task: str, fold: int, blocks: list[dict[str, Any]], device: torch.device) -> dict[str, Any]:
    ...
    row, checkpoint = pswa.checkpoint_row("EEGNet", task, fold)
    ...
    h_source = peeh.representations(net, head, capped["source_x"], "EEGNet", device, batch=32)
    h_future = peeh.representations(net, head, capped["future_x"], "EEGNet", device, batch=32)
    ...
    spec = peeh.spectrum(h, y, owner, sessions, task, "EEGNet", fold)
...
def freeze_selectors(task: str, fold: int, device: torch.device) -> dict[str, Any]:
    ...
    frozen = {
        "schema": "P2_EEGNET_FROZEN_SELECTORS_V1", "Model": "EEGNet", "Task": task,
...
def evaluate_frozen(frozen: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for selector in ("PERSISTENCE_ONLY", "UTILITY_ONLY", "JOINT"):
        ...
        for subject, values in by_subject.items():
            rows.append({"Model": "EEGNet", "Task": frozen["Task"], "fold": frozen["fold"], "seed": 0,
...
def run_cell(task: str, fold: int, dry_run: bool = False) -> dict[str, Any]:
    ...
    result = {
        "schema": "P2_EEGNET_CELL_V1", "Model": "EEGNet", "Task": task, "fold": fold, "seed": 0,
...
def aggregate() -> None:
    ...
    valid = {
        "pass": True, "model_training_count": 0, "neural_finetuning_count": 0, "tasks": 4,
        "folds_per_task": 5, "seed": 0, "all_20_cells_accounted": len(cells) == 20,
        "joint_exactly_matches_corrected_peeh": all(x["K"] == len(x["selectors"]["JOINT"]["coordinates"]) for x in cells),
    ...
    valid["pass"] = all(v for k, v in valid.items() if k.startswith("all_") or k.startswith("joint_") or k.startswith("no_"))
    ...
    print("P2_SELECTION_FACTOR_EEGNET_20_CELLS_COMPLETE", flush=True)


def render_report(rows: list[dict[str, Any]]) -> None:
    lines = ["# P2 EEGNet selection-factor ablation", "", "Scope: frozen EEGNet seed 0, four tasks, five folds. Neural training and fine-tuning count: **0**.",
    ...
    (OUT / "FINAL_P2_EEGNET_REPORT.md").write_text("\n".join(lines) + "\n", encoding="utf-8")


def write_protocol() -> None:
    write_json(PROTOCOL / "PROTOCOL.json", {
        "schema": "P2_SELECTION_FACTOR_EEGNET_PROTOCOL_V1", "model": "EEGNet", "tasks": list(TASKS),
    ...
    write_json(PROTOCOL / "SELECTOR_DEFINITIONS.json", {
