...
The protocol and global discovery locks are written under `protocol/`. Required aggregated tables and the report are written under `outputs/`. The full `SUBJECT_DIRECTION_UTILITY.csv` is 143 MB and is retained uncompressed in the experiment workspace and server runtime; the Git branch stores its lossless `SUBJECT_DIRECTION_UTILITY.csv.gz` copy. Decompress it to recover the required CSV. Per-cell artifacts and completion hash manifests live in the runtime directory.
