"""Matched B0/LiteBN-G evaluation on already-exposed development benchmarks."""
...
def evaluate_task(*, base, runner, task: str, folds: list[dict], checkpoints: dict[int, Path], device: torch.device, output: Path) -> tuple[dict, pd.DataFrame]:
    ...
    for fold in folds:
        ...
        g = LiteBNG(base.build_model("LiteBN_BASELINE", task)); payload = torch.load(checkpoints[fold_id], map_location="cpu", weights_only=False); g.load_state_dict(payload["state_dict"], strict=True); g = g.to(device).eval()
        for name, model, path in (("B0", b0, b0_path), ("LiteBN-G", g, checkpoints[fold_id])):
    ...
    if set(aggregate.columns) != {"B0", "LiteBN-G"}: raise RuntimeError("incomplete matched evaluation")
    b0, g = float(aggregate.B0.mean()), float(aggregate["LiteBN-G"].mean()); reference, reference_model = BENCHMARK[task]
    result = {"task": task, "scope": "EXPOSED_DEVELOPMENT_BENCHMARK", "resource": "OpenBMI_V8_INTERNAL" if task.startswith("OpenBMI") else "WBCIC_FINAL_TRUE_OUTER_EXPOSED_DEVELOPMENT",
              ...
              "matched_LiteBN_BA": b0, "LiteBN_G_BA": g, "delta_vs_LiteBN_pp": 100 * (g - b0), "delta_vs_benchmark_pp": 100 * (g - reference)}
